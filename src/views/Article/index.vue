<template>
  <div class="article-container">
    ...
    <!-- 再vue组件中，把一些和当前组件无关不属于当前文档流中的东西放在最后面 -->
    <!--  v-if="addCommentShow":每次关闭，组件销毁；每次打开，组件重新创建，mounted就会再一次执行 -->
...
export default {
  ...
  data () {
    return {
      ...
      article: {}, // 初始化对象如果是数字则null、字符串null，数组[],duixiang {}
      is404Error: false,
      showShare: false,
      options: [
        { name: '微信', icon: 'wechat' },
  ...
  async created () {
    ...
    // 在这里loading关闭，页面就显示正文，但是必须要写在nextTick里
    // 因为数据更新，视图不能立即更新，写在nextTick回调中可以，nextTick回调就是在视图更新完成才会触发
    this.$nextTick(() => {
      // 获取正文所有的图片
      // 步骤
      // 1.通过querySelectorAll获取正文中所有图片
      // 2.对图片数组进行foreach遍历，先把图片src地址放在arr1里
      // 3.给所有的图片dom对象绑定点击事件，点击的时候预览效果出现
